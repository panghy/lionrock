...
/**
 * In-memory implementation of {@link TransactionalKeyValueStoreGrpc} that allows only one transaction to go through
 * at a single time.
 *
 * @author Clement Pang
 */
...
public class InmemoryServer extends TransactionalKeyValueStoreGrpc.TransactionalKeyValueStoreImplBase {
  ...
  public static void main(String[] args) {
    SpringApplication.run(InmemoryServer.class, args);
  }

  private final LoadingCache<String, InmemoryDatabase> databaseMap = CacheBuilder.newBuilder().build(
      new CacheLoader<>() {
        @Override
        public InmemoryDatabase load(String key) {
  ...
  @PostConstruct
  public void init() {
    logger.info("Starting up in-memory lionrock server");
  }
  ...
  public void execute(DatabaseRequest request, StreamObserver<DatabaseResponse> responseObserver) {
    ...
    Database db = databaseMap.getUnchecked(request.getDatabaseName());
...
/*  @Override
  ...
  private void prepareTx(DatabaseRequest databaseRequest, Transaction tx) {
